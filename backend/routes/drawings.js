...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!doodle.isPublic) {
      return res.status(404).json({
        status: 'error',
        message: 'Doodle is not available'
      });
    }
...
router.post('/', [
  ...
  try {
    ...
    const newlyUnlockedBadges = await BadgeService.checkDoodlesCompletedBadges(req.user.id);
...
router.put('/:id/like', protect, async (req, res) => {
  try {
    ...
    // Check for badge unlocks after liking
    const newlyUnlockedBadges = await BadgeService.checkLikesReceivedBadges(doodle.userId);
...
router.post('/:id/rate', [
  ...
  try {
    ...
    // Check for badge unlocks after rating
    const newlyUnlockedBadges = await BadgeService.checkPerfectRatingsBadges(doodle.userId);
