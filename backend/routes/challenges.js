    ...
    res.status(200).json({
      ...
      data: {
        ...
        pagination: {
          ...
          pages: Math.ceil(totalChallenges / filters.limit)
        }
      }
    });
  } catch (error) {
...
/**
 * @route   GET /api/challenges/:id/doodles
 * @desc    Get all doodles for a specific challenge
 * @access  Public
 */
router.get('/:id/doodles', [
  ...
  try {
    ...
    res.status(500).json({
      status: 'error',
      message: 'Failed to fetch challenge doodles'
    });
  }
});
 ...
 * @desc    Submit a doodle for a challenge
 * @access  Private
 */
router.post('/:id/submit', [
  ...
  try {
    ...
    // Populate user and challenge info
    ...
    res.status(500).json({
      status: 'error',
      message: 'Failed to submit doodle'
    });
  }
});
 ...
 * @desc    Get all available challenge categories
 * @access  Public
 */
...
/**
 * @route   GET /api/challenges/difficulties
 * @desc    Get all available difficulty levels
 * @access  Public
 */
...
router.get('/random', [
  ...
  try {
    ...
    if (!challenge || challenge.length === 0) {
      return res.status(404).json({
        status: 'error',
        message: 'No challenges found with the specified criteria'
      });
    }
